...
case "${ARCH}" in \
   ...
   *) \
     ...
     ;; \
      ...
      curl -fsSL https://github.com/BurntSushi/ripgrep/releases/download/${RG_VERSION}/ripgrep-${RG_VERSION}-${ripgrepArch}-unknown-linux-${ripgrepClib}.tar.gz | tar --extract --gunzip --strip-components=1 --verbose --directory /usr/local/bin/ ripgrep-${RG_VERSION}-${ripgrepArch}-unknown-linux-${ripgrepClib}/rg; \
